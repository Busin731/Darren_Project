...
const Pagination = (props: MPaginationProps) => {    
    const { rowsPerPageOptions, actions, limit, ...rest } = props;
    const { topage, isurl } = rest;
    const {
        loading,
        page,
        perPage,
    ...
    React.useEffect(() => {
        if (topage > 1 && isurl === 'false') setPage(topage);
    }, [setPage, topage, isurl])
...
// @ts-ignore
export default React.memo<MPaginationProps>(Pagination);
